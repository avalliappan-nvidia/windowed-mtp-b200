...
def extend_attention_fwd(
    ...
    import os as _os
    if _os.environ.get("RK_DEBUG_KVLEN") and not torch.cuda.is_current_stream_capturing():
        # Actual prefix (KV) scan length per request as seen by _fwd_kernel.
        # NOTE: draft-extend is CUDA-graphed (Device2ExtendCudaGraphRunner), so
        # this only fires on an EAGER call (never during capture -- a D2H copy
        # there is illegal). Under graphed decode this prints nothing.
        _kvp = kv_indptr[: batch_size + 1].to("cpu")
        _kvlen = (_kvp[1:] - _kvp[:-1]).tolist()
        _qop = qo_indptr[: batch_size + 1].to("cpu")
        _qolen = (_qop[1:] - _qop[:-1]).tolist()
        print(
            f"[RK_DEBUG_KVLEN] bs={batch_size} kv_scan_len={_kvlen} "
            f"qo_len(extend/AL)={_qolen} max_len_extend={int(max_len_extend)} "
            f"sliding_window_size={int(sliding_window_size)} "
            f"kv_indices_numel={int(kv_indices.numel())}",
            flush=True,
        )

    grid = (batch_size, head_num, triton.cdiv(max_len_extend, BLOCK_M))
    # [RK experiment] pipeline depth is knob-tunable; default 1 (upstream). Set
    # RK_EXTEND_STAGES=2 to overlap KV loads with the MMA (see ubenchmark E4).
    import os as _os
    num_stages = int(_os.environ.get("RK_EXTEND_STAGES") or "1")
